  ...
  return (
    <nav 
      className={`
        ...
        ${scrolled 
          ? 'h-20 bg-gray-900/80 backdrop-blur-md border-gray-700/50 shadow-lg shadow-black/20' 
          : 'h-24 bg-gray-900 border-gray-700'
        }
      `}
    >
      {/* Partículas con opacidad dinámica */}
      <div className={`transition-opacity duration-300 ${scrolled ? 'opacity-30' : 'opacity-100'}`}>
        <ParticlesBackground />
      </div>

      {/* Logo (izquierda) con animación de escala */}
      <NavLink to="/" className={`flex items-center gap-3 transition-transform duration-300 ${scrolled ? 'scale-95' : 'scale-100'}`}>
        <img src={Logo} alt="Infinity Academia Logo" className={`w-auto transition-all duration-300 ${scrolled ? 'h-8' : 'h-10'}`} />
        <div className="flex flex-col leading-none">
          <div className="flex items-center gap-1">
            <span className={`font-custom-montserrat text-transparent bg-clip-text bg-gradient-to-r from-gray-200 via-gray-400 to-gray-600 transition-all duration-300 ${scrolled ? 'text-2xl' : 'text-3xl'}`}>
              INFINITY
            </span>
          </div>
          <span className={`font-custom-montserrat text-gray-300 text-center transition-all duration-300 ${scrolled ? 'text-lg' : 'text-xl'}`}>
            Academia
          </span>
        </div>
      </NavLink>
      ...
      <ul className={`hidden xl:flex items-center gap-10 absolute left-1/2 -translate-x-1/2 transition-all duration-300 ${scrolled ? 'text-xs' : 'text-xs 2xl:text-lg'}`}>
        {[
          ...
          <li key={item.name} className="relative group">
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  ...
                  // color gris con brillo sutil más intenso cuando está scrolled
                  scrolled 
            ...
            <span className={`pointer-events-none absolute -bottom-1 left-0 h-[2px] w-full rounded-full origin-left scale-x-0 transition-all duration-200 group-hover:scale-x-100 ${scrolled ? 'bg-gray-100' : 'bg-gray-300'}`} />
          </li>
        ))}
      </ul>

      {/* CTA derecha (desktop) con animación */}
      <div className={`ml-auto hidden xl:flex items-center gap-6 transition-all duration-300 ${scrolled ? 'text-xs' : 'text-xs 2xl:text-lg'}`}>
        <div className={`flex items-center transition-all duration-300 ${scrolled ? 'gap-3' : 'gap-4'}`}>
          {socialLinks.map((link) => (
            <a
              ...
              className={`transition-all duration-200 ${scrolled ? 'text-gray-200 hover:text-white hover:scale-110' : 'text-gray-300 hover:text-white'}`}
            >
      ...
      <div 
        onClick={handleNav} 
        className={`block xl:hidden ml-auto z-50 transition-all duration-300 ${scrolled ? 'hover:scale-110' : ''}`}
      >
        {nav ? <AiOutlineClose size={scrolled ? 22 : 24} /> : <AiOutlineMenu size={scrolled ? 22 : 24} />}
      </div>
      ...
      <div
        className={
          nav
            ? "fixed xl:hidden left-0 top-0 w-full h-full bg-gray-900/95 backdrop-blur-md ease-in-out duration-300 z-40"
      ...
      >
        ...
        >
          ...
          <FaInfinity className="text-3xl text-gray-400 drop-shadow-[0_0_6px_rgba(200,200,200,0.4)]" />
        </NavLink>
